...
public class JoinActDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    {
        Dal_JAA dal_jaa = DataManager.getInstance().getM_dataJAA();
        JoinActApply joinActApply =dal_jaa.GetData(m_nPull);
        Student st = DataManager.getInstance().getM_dataStudent().GetData(joinActApply.getApplyer());
        if(st != null)
        {
            x.image().bind(m_ivPhoto, Utils.configFileUrl(NetProtocol.FILEHANDLE_USER,st.getId(),"0.jpg"));
            m_tvName.setText(st.getName());
            m_eOrgName.setText(DataManager.getInstance().getM_dataConfig().getM_listClass().get(st.getCollege()));
            m_eOrgId.setText(st.getGrade()+"级");
            m_eOrgTitle.setText("普通成员");
            String strState = joinActApply.getOptype()==NetProtocol.JOINSTATE_ADD?"申请加入":"申请退出";
            m_eOrgState.setText(strState);
            m_eOrgReason.setText(joinActApply.getReason());
        }
        else
        {
           onPullStudentDetailClick(joinActApply.getApplyer());
        }
    }
    ...
    private void onApproveJoinActApplyClick(int m_state) throws DbException {
        RequestParams params = new RequestParams(NetProtocol.NETROOT + NetProtocol.APPROVEJOINACT);
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                NetHandler hd = netManager.GetNetHandler(NetProtocol.APPROVEJOINACT);
    ...
    public void ApproveActApply(MessageEventApproveJoinAct message){
        m_nErrorCode = message.getnErroecode();
        if (m_nErrorCode == 0) {
            this.finish();
    ...
    private void onPullStudentDetailClick(int id) {
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                Bundle bundle = new Bundle();
                bundle.putString("result", result);
                bundle.putInt("type", NetProtocol.PULLALLID_APPLY_JOIN_ACT);
                message.setData(bundle);
    ...
    public void getUserDetail(MEUserDetails data) {
        ...
        if(data.getM_nType() != NetProtocol.PULLALLID_APPLY_JOIN_ACT)
            return;
